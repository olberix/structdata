...
```
这是基于外存实现的B树，HeaderNode存储了B树的元信息，位于文件开头并独占一页空间，其中记录包括根结点，键值大小和页大小；BNode是B树结点结构，存储了关键字个数，键值对，孩子指针和自身指针，自身指针不存入文件，在写入文件或从文件读取时才被赋值；BTree是B树结构，包含了比较函数，文件描述符，最大最小关键字数等信息。  
